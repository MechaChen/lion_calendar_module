...
class Calendar extends React.Component {
  ...
  getMonthSchedules = () => {
    const { schedules, months, targetMonth } = this.state;
    return (
      schedules
        .filter(schedule =>
  ...
  componentDidMount() {
    ...
    // const formedInitYearMonth = `${initYearMonth.substr(0, 4)}-${initYearMonth.substr(4)}`;
    // get all schedules
    if(typeof dataSource === 'string') {
      axios.get(this.props.dataSource).then(({ data }) => {
        ...
        this.setState(() => ({ schedules, months, ...targetMonths }));
      });
    } else {
...
window.calendar = ReactDOM.render(<Calendar />, document.getElementById("calendar"));
